...
const LINUX_DESKTOP_ENTRY_NAME = isDevelopment ? "k3code-dev.desktop" : "k3code.desktop";
const LINUX_WM_CLASS = isDevelopment ? "k3code-dev" : "k3code";
const USER_DATA_DIR_NAME = isDevelopment ? "k3code-dev" : "k3code";
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("k3code failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `k3code ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/k3code (Alpha)` on Linux). This is
